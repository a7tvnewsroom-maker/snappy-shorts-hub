import { Home, Film, TrendingUp, Bell, User, Menu, Plus } from "lucide-react";
...
const Header = ({ onMenuClick, onCreateClick }: HeaderProps) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-card border-b border-border safe-area-top">
      {/* Desktop Layout */}
      <div className="hidden md:flex items-center justify-between max-w-[600px] mx-auto px-4 py-3">
      ...
      <nav className="hidden md:flex items-center justify-center max-w-[600px] mx-auto">
        {tabs.map((tab) => {
          const Icon = tab.icon;
      ...
      {/* Navigation tabs - Mobile full width */}
      <nav className="md:hidden flex items-center px-2">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          const isActive = activeTab === tab.id;
          return (
